<template>
  ...
  >
    ...
    >
      <div class="article-detail" style="border-radius: 2%;">
        <!--标题-->
        <div class="title">
        ...
        <div class="user-info">
          ...
          <div class="user-info-detail">
            ...
            <div class="time-info">
              ...
              <router-link
                v-if="articleInfo.userId == userInfo.userId"
        ...
        <!--文章详情-->
        <div class="detail" id="detail" v-html="articleInfo.content" style="font-family: kaiti;margin: 40px 35px;"></div>
      </div>
      <!--附件-->
      <div class="attachment-panel" v-if="attachment" id="view-attachment" style="border-radius: 2%;">
...
const getArticleDetail = (articleId) => {
  try {
    const response = axios.post(proxy.globalInfo.url_f+'/forum/getArticleDetail', {
...
axios.post(proxy.globalInfo.url_f+'/forum/read', {
...
const doLikeHandler = async () => {
  try {
    const response = await axios.post(proxy.globalInfo.url_f+'/forum/doLike', {
      articleId: articleInfo.value.articleId,
      userId: userInfo.value.userId  // 请替换为实际的用户ID
    });
...
const downloadAttachment = async (fileId) => {
  ...
  if (
    ...
    userInfo.value.userId == articleInfo.value.userId
  ) {
    downloadDo(fileId);
    return;
  }
  ...
  try {
    const response = await axios.post(proxy.globalInfo.url_f+'/forum/getUserDownloadInfo', {
      fileId: fileId,
      userId: userInfo.value.userId  // 请替换为实际的用户ID
    });
    ...
    console.error('Error while fetching user download info:', error);
  }
};
